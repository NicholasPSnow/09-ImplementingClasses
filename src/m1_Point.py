...
class Point(object):
    ...
    def move_to (self,x,y):
        import math
        self.distance = self.distance + math.sqrt(abs(self.x - x) ** 2 + abs(self.y - y) ** 2)
        self.x=x
        self.y=y
    ...
    def move_by (self,dx,dy):
        ...
        self.y=self.y+dy
        self.moves = self.moves + 1
        self.distance=self.distance+math.sqrt(abs(dx)**2+abs(dy)**2)
    ...
    def get_distance_from_start(self):
        import math
        return math.sqrt(abs(self.x - self.initalpointx) ** 2 + abs(self.y - self.initalpointy) ** 2)
    ...
    def closer_to(self,p2,p3):
        import math
        p2vsself =math.sqrt(abs(self.x - p2.x) ** 2 + abs(self.y - p2.y) ** 2)
        p3vsself =math.sqrt(abs(self.x - p3.x) ** 2 + abs(self.y - p3.y) ** 2)

        if (p2vsself==p3vsself):
            return p2
        if (p2vsself >= p3vsself):
            return p3
        if (p2vsself <= p3vsself):
